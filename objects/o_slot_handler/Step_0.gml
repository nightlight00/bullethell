...
if (delay == 0 && !instance_exists(o_enembase)) {
	...
	for (var i = 0; i < instance_number(o_slotpanel); i++) {
		...
		if (slot.image_speed > 0) {
			...
			if (global.player_level mod 6 == 5) {
				slot.image_index = 3;
			}
	...
	if (global.player_level mod 6 == 5) {
		...
		spawnType[3] = 3 + floor(global.player_level / 6);
	}
	
	// spawnType[3] = JACKPOT (3 for all other slot types)
	if (spawnType[3] >= 2) {
		for (var d = 0; d < spawnType[3]; d++) {
			for (var i = 0; i < slotStyles - 1; i++) {
				spawnType[i] ++;
			}
		}
	}
	...
	if (amount > 0) {
		amount += floor(global.player_level / 5);
	}
	while (amount > 0) {
